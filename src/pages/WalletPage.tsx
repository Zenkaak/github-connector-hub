...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { font-family: 'Segoe UI', Arial, sans-serif; color: #1a1a2e; font-size: 10px; line-height: 1.5; }
  ...
  .brand { font-size: 18px; font-weight: 900; color: #0a1628; letter-spacing: 1px; }
  .brand-sub { font-size: 7px; color: #d4a853; font-weight: 700; text-transform: uppercase; letter-spacing: 2px; }
  ...
  .title { font-size: 12px; font-weight: 800; text-transform: uppercase; letter-spacing: 1px; color: #0a1628; margin-bottom: 4px; }
  .period { font-size: 9px; color: #666; font-weight: 600; }
  ...
  .info-label { font-size: 7px; font-weight: 800; text-transform: uppercase; letter-spacing: 1px; color: #888; }
  .info-value { font-size: 14px; font-weight: 900; color: #0a1628; margin-top: 2px; }
  .info-value.green { color: #059669; }
  .info-value.red { color: #dc2626; }
  table { width: 100%; border-collapse: collapse; margin-top: 16px; }
  thead th { background: #0a1628; color: #fff; font-size: 7px; font-weight: 800; text-transform: uppercase; letter-spacing: 1px; padding: 10px 12px; text-align: left; }
  tbody td { padding: 9px 12px; border-bottom: 1px solid #eef0f4; font-size: 9px; }
  ...
  .footer-text { font-size: 7px; color: #999; }
  .seal { width: 50px; height: 50px; border: 2px solid #d4a853; border-radius: 50%; display: flex; align-items: center; justify-content: center; text-align: center; }
  .seal-text { font-size: 5px; font-weight: 900; color: #d4a853; text-transform: uppercase; letter-spacing: 0.5px; line-height: 1.2; }
  .user-info { margin-bottom: 20px; }
  .user-name { font-size: 12px; font-weight: 800; }
  .user-detail { font-size: 9px; color: #666; }
</style>
</head>
<body>
  <div class="header">
  ...
  if (loading) return <DashboardLayout><div className="h-screen flex items-center justify-center"><Loader2 className="animate-spin text-accent" size={24} /></div></DashboardLayout>;
  ...
  return (
    <DashboardLayout>
      <div className="p-4 lg:p-8 space-y-6 max-w-[1400px] mx-auto">
          ...
          <Card className="premium-card border-none bg-gradient-to-br from-[hsl(var(--navy-800))] via-[hsl(var(--navy-700))] to-[hsl(var(--navy-900))] overflow-hidden relative">
            ...
            <CardContent className="p-6 md:p-8 relative z-10">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <p className="text-[9px] font-semibold text-foreground/40 uppercase tracking-[0.2em] mb-1.5">Available Balance</p>
              ...
              <div className="grid grid-cols-4 gap-2">
                {[
                  { label: 'Send', icon: Send, action: () => setSendMoneyOpen(true) },
                  ...
                  >
                    <div className={cn(
                      ...
                      <btn.icon size={14} className={i === 0 ? "text-accent" : "text-foreground/70"} />
                    </div>
                    <span className={cn("text-[9px] font-semibold", i === 0 ? "text-accent" : "text-foreground/60")}>{btn.label}</span>
                  </button>
                ))}
              </div>
            </CardContent>

            <div className="border-t border-foreground/5 px-6 py-2 flex items-center justify-between">
              <div className="flex items-center gap-2 text-foreground/30">
                <ShieldCheck size={10} />
                <span className="text-[8px] font-semibold uppercase tracking-[0.15em]">Secured by DASNET</span>
              </div>
              <span className="text-[8px] font-mono text-foreground/20">{wallet?.id.slice(0, 12).toUpperCase()}</span>
            </div>
          </Card>
        </motion.div>
            ...
            <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 * i }}>
              <Card className={cn("border bg-card/80 backdrop-blur-sm", m.border)}>
                <CardContent className="p-3 md:p-3">
                  ...
                  <p className="text-sm md:text-base font-bold text-foreground tracking-tight">{formatCurrency(m.val)}</p>
                </CardContent>
              </Card>
            </motion.div>
          ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-8">
            <Card className="border-border/40 overflow-hidden">
              <CardHeader className="p-4 border-b border-border/30 bg-card/50">
                <div className="flex flex-col gap-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <CardTitle className="text-xs font-bold">Transactions</CardTitle>
                        <CardDescription className="text-[9px] mt-0">{transactions.length} items</CardDescription>
                      </div>
                    </div>
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="sm" onClick={fetchWalletData} className="h-6 w-6 p-0 rounded-lg text-muted-foreground hover:text-foreground">
                        <RefreshCw size={12} />
                      </Button>
                      <Button variant="outline" size="sm" onClick={exportStatement} className="h-6 rounded-lg text-[9px] gap-1 font-semibold px-2">
                        <Download size={10} /> CSV
                      </Button>
                      <Button variant="outline" size="sm" onClick={() => setStatementPeriod(-1)} className="h-6 rounded-lg text-[9px] gap-1 font-semibold px-2 text-accent border-accent/30">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    <div className="relative flex-1">
                      ...
                      <Input
                        placeholder="Search..."
                        className="pl-8 h-7 bg-muted/40 border-border/30 rounded-lg text-[11px]"
                        value={searchQuery}
                    ...
                    <Tabs value={filterType} onValueChange={(v: any) => setFilterType(v)}>
                      <TabsList className="h-7 bg-muted/50 rounded-lg p-0.5">
                        <TabsTrigger value="all" className="text-[9px] rounded-md px-2.5 h-6 data-[state=active]:bg-card font-semibold">All</TabsTrigger>
                        <TabsTrigger value="in" className="text-[9px] rounded-md px-2.5 h-6 data-[state=active]:bg-emerald-500/15 data-[state=active]:text-emerald-400 font-semibold">In</TabsTrigger>
                        <TabsTrigger value="out" className="text-[9px] rounded-md px-2.5 h-6 data-[state=active]:bg-rose-500/15 data-[state=active]:text-rose-400 font-semibold">Out</TabsTrigger>
                      </TabsList>
                    </Tabs>
                  </div>
                </div>
              ...
              <CardContent className="p-0">
                {filteredTransactions.length === 0 ? (
                  <div className="py-12 flex flex-col items-center">
                    <Receipt size={20} className="text-muted-foreground/30 mb-2" />
                    <p className="text-xs font-semibold text-muted-foreground">No records</p>
                  </div>
                ) : (
                    ...
                    {filteredTransactions.map((tx, idx) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className="px-4 py-3 flex items-center justify-between hover:bg-muted/10 cursor-pointer transition-all"
                        >
                          <div className="flex items-center gap-3">
                            <div className={cn(
                              "w-8 h-8 rounded-lg flex items-center justify-center",
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-1.5 mt-0.5">
                                ...
                                <Badge className={cn("text-[7px] font-bold px-1 h-3.5", statusInfo.className)}>
                                  {statusInfo.label}
                                </Badge>
                              </div>
                            </div>
                          ...
                          <div className="text-right">
                            <p className={cn("text-[12px] font-bold tabular-nums", isIncoming(tx) ? "text-emerald-400" : "text-foreground")}>
            ...
            <Card className="border-border/40 overflow-hidden">
              <div className="bg-gradient-to-br from-accent/5 to-transparent p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold text-foreground">DASNET Wallet</p>
                    <p className="text-[9px] text-muted-foreground">Financial Security</p>
                  </div>
                </div>
              </div>
              <CardContent className="p-4 pt-0 mt-2">
                <div className="space-y-2">
                  {[
                    ...
                    { icon: Smartphone, text: 'Instant M-Pesa' },
                    { icon: Lock, text: 'Encrypted logs' },
                  ].map((f, i) => (
            ...
            <Card className="border-border/40">
              <CardHeader className="p-4 border-b border-border/30 flex flex-row items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-xs font-bold">Pending</CardTitle>
                </div>
                <Badge variant="outline" className="text-[9px] font-semibold h-4 min-w-[16px] justify-center">
              ...
              <CardContent className="p-0">
                {withdrawals.filter(w => w.status === 'pending').length === 0 ? (
                  <div className="p-4 text-center">
                    <p className="text-[9px] text-muted-foreground">No pending requests</p>
                  </div>
                ) : (
                  <div className="divide-y divide-border/10">
                    {withdrawals.filter(w => w.status === 'pending').map((wd) => (
                      <div key={wd.id} className="p-3 space-y-1.5">
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="text-sm font-bold text-foreground">{formatCurrency(wd.amount)}</p>
                            <p className="text-[9px] text-muted-foreground">{wd.phone}</p>
                          </div>
                          <Badge className="bg-accent/10 text-accent border-accent/20 text-[8px] font-semibold">Processing</Badge>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <Card className="border-border/40">
              <CardHeader className="p-4 border-b border-border/30 flex flex-row items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-xs font-bold">Transfers</CardTitle>
                </div>
                <Badge variant="outline" className="text-[9px] font-semibold h-4">{transfers.length}</Badge>
              </CardHeader>
              <CardContent className="p-0 max-h-[240px] overflow-y-auto">
                {transfers.length === 0 ? (
                  <p className="p-4 text-center text-[9px] text-muted-foreground">No transfers</p>
                ) : (
                  <div className="divide-y divide-border/10">
                    {transfers.map((tr) => (
                      <div key={tr.id} onClick={() => setSelectedTransfer(tr)} className="p-3 flex items-center justify-between hover:bg-muted/10 transition-colors cursor-pointer">
                        <div className="flex items-center gap-2">
                          <div className={cn(
                          ...
                          <div>
                            <p className="text-[11px] font-semibold text-foreground">{tr.sender_id === user?.id ? tr.receiver_name : tr.sender_name}</p>
                            <p className="text-[8px] text-muted-foreground">{new Date(tr.created_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}</p>
                          </div>
                        </div>
                        <p className={cn("text-[11px] font-bold", tr.sender_id === user?.id ? "text-rose-400" : "text-emerald-400")}>
        ...
        {/* Dialogs with Reduced Font Sizes */}
        <Dialog open={withdrawDialog} onOpenChange={setWithdrawDialog}>
          <DialogContent className="sm:max-w-md rounded-2xl p-0 overflow-hidden">
            <div className="bg-gradient-to-br from-[hsl(var(--navy-800))] to-[hsl(var(--navy-900))] p-5">
            ...
            <div className="p-5 space-y-4">
              <div className="p-3 rounded-xl bg-muted/30 border border-border/30 text-center">
                ...
                <p className="text-xl font-bold text-accent">{formatCurrency(wallet?.balance || 0)}</p>
              </div>
              <div className="space-y-3">
                <div className="space-y-1">
                  <Label className="text-[11px] font-semibold">Amount (KES)</Label>
                  <Input type="number" value={withdrawAmount} onChange={(e) => setWithdrawAmount(e.target.value)} placeholder="Min 100" className="h-10 rounded-lg text-sm font-semibold" />
                </div>
                <div className="space-y-1">
                  <Label className="text-[11px] font-semibold">M-Pesa Number</Label>
                  <Input value={registeredPhone} readOnly className="h-10 rounded-lg font-semibold bg-muted/50 text-sm" />
                </div>
              </div>
            </div>
            <div className="p-4 bg-muted/10 border-t border-border/20 flex gap-2">
              <Button variant="ghost" onClick={() => setWithdrawDialog(false)} className="flex-1 h-9 rounded-lg text-xs font-semibold">Cancel</Button>
              <Button variant="gold" onClick={handleWithdraw} disabled={actionLoading} className="flex-1 h-9 rounded-lg text-xs font-semibold">Confirm</Button>
            </div>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={depositDialog} onOpenChange={(open) => { if (!open) resetDepositState(); else setDepositDialog(true); }}>
          <DialogContent className="sm:max-w-md rounded-2xl p-0 overflow-hidden">
            ...
            <div className="p-5 space-y-4">
              {depositStatus === 'success' ? (
                <div className="text-center py-2 space-y-2">
                  ...
                  <Button onClick={resetDepositState} variant="gold" className="h-8 text-xs rounded-lg">Done</Button>
                </div>
              ) : (
                <div className="space-y-3">
                  <div className="space-y-1">
                    <Label className="text-[11px] font-semibold">Amount</Label>
                    <Input type="number" value={depositAmount} onChange={(e) => setDepositAmount(e.target.value)} className="h-10 rounded-lg text-sm" />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-[11px] font-semibold">M-Pesa Number</Label>
                    <Input value={registeredPhone} readOnly className="h-10 rounded-lg bg-muted/50 text-sm" />
                  </div>
                </div>
              )}
            </div>
            {depositStatus !== 'success' && (
              <div className="p-4 bg-muted/10 border-t border-border/20 flex gap-2">
                <Button variant="ghost" onClick={resetDepositState} className="flex-1 h-9 rounded-lg text-xs font-semibold">Cancel</Button>
                <Button variant="gold" onClick={handleDeposit} disabled={actionLoading} className="flex-1 h-9 rounded-lg text-xs font-semibold">Send STK</Button>
              </div>
            )}
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={!!selectedTx} onOpenChange={() => setSelectedTx(null)}>
          <DialogContent className="sm:max-w-md rounded-2xl p-0 overflow-hidden border-border/30">
            {selectedTx && (() => {
              ...
              return (
                <>
                  <div className="bg-gradient-to-br from-[hsl(var(--navy-800))] via-[hsl(var(--navy-700))] to-[hsl(var(--navy-900))] p-6 text-center">
                    <p className="text-[9px] text-foreground/40 uppercase tracking-widest mb-1">Receipt</p>
                    <p className={cn("text-2xl font-bold", isIncoming(selectedTx) ? "text-emerald-400" : "text-foreground")}>
                      {isIncoming(selectedTx) ? '+' : '-'} {formatCurrency(selectedTx.amount)}
                    </p>
                    <div className="mt-2 flex justify-center gap-1.5">
                      <Badge className="text-[8px] font-bold px-2 py-0 h-4">{getTransactionLabel(selectedTx)}</Badge>
                    </div>
                  </div>
                  <div className="p-5 space-y-3">
                    {[
                      { label: 'Transaction ID', value: selectedTx.id.slice(0, 12).toUpperCase() },
                      { label: 'Date', value: new Date(selectedTx.created_at).toLocaleString() },
                      { label: 'Description', value: selectedTx.description },
                      ...(selectedTxExtra?.mpesa_receipt ? [{ label: 'M-Pesa Ref', value: selectedTxExtra.mpesa_receipt }] : []),
                    ].map((item, i) => (
                      <div key={i} className="flex justify-between py-1.5 border-b border-border/5 last:border-0">
                        <span className="text-[10px] text-muted-foreground font-medium">{item.label}</span>
                        <span className="text-[10px] font-semibold text-foreground">{item.value}</span>
                      </div>
                    ))}
                    <Button variant="outline" className="w-full rounded-lg h-9 text-xs font-semibold mt-2" onClick={() => setSelectedTx(null)}>Close</Button>
                  </div>
                </>
              );
            })()}
        ...
        <Dialog open={statementPeriod !== null} onOpenChange={(open) => { if (!open) setStatementPeriod(null); }}>
          <DialogContent className="sm:max-w-sm rounded-2xl p-0 overflow-hidden">
            <div className="bg-gradient-to-br from-[hsl(var(--navy-800))] to-[hsl(var(--navy-900))] p-5">
              <DialogTitle className="text-base font-bold text-foreground">Statement Period</DialogTitle>
            </div>
            <div className="p-5 space-y-2">
              {[3, 6, 12].map(m => (
                <Button key={m} variant="outline" className="w-full h-11 rounded-lg justify-between text-xs font-bold" onClick={() => generatePDFStatement(m)} disabled={generatingStatement}>
                  <span>{m} Months</span>
                  <Download size={12} />
                </Button>
              ))}
